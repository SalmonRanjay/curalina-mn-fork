...
| `category` | 6 `Furniture Category` | Comma-delimited multi-value, so a row maps to *several* categories — `Product.category` is a single string, so the importer needs a primary-category rule. **Unresolved by the data**: zero rows use `Coffee Table` despite the taxonomy defining it. One junk value (`Final CSV-Ready Summary`). ~~Two rows blank~~ **Correction (post-ADR, verified twice independently — by the R01 notebook's own re-derivation and separately by direct spot-check against the pinned file): zero rows have a blank category, not two.** The "two blank" figure in the original ruling was wrong; it did not change the ruling (R01 still proceeds, R03 still blocked) and no other claim in this ADR depended on it. R01 must still recommend a primary-category rule for the genuine multi-value case; it must not invent a default silently. |
