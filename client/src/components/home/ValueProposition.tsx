import { Check } from "lucide-react";
import beforeAfterImage from "@assets/stock_images/beautiful_modern_liv_cc3ef764.jpg";

const benefits = [
  "Every room is handpicked by professional interior designers",
  "Hundreds of premium brands — your design isn't limited to one store",
  "You get shoppable designs — what you see is exactly what you can get",
];

export default function ValueProposition() {
  return (
    <section className="bg-white py-16 md:py-24" data-testid="section-value-proposition">
      <div className="max-w-[1120px] mx-auto px-6 md:px-12 lg:px-16">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          {/* Left - Text Content */}
          <div className="space-y-6" data-testid="value-content">
            <h2 className="text-3xl md:text-4xl font-cormorant font-medium text-stone-900" data-testid="heading-value-prop">
              Designed by humans, powered by smart tools
            </h2>
            
            <ul className="space-y-4">
              {benefits.map((benefit, index) => (
                <li key={index} className="flex gap-3" data-testid={`benefit-${index + 1}`}>
                  <div className="flex-shrink-0 w-6 h-6 bg-[#D9F2E5] rounded-full flex items-center justify-center mt-0.5">
                    <Check className="w-4 h-4 text-[#24A8AE]" />
                  </div>
                  <span className="text-base md:text-lg text-stone-700">{benefit}</span>
                </li>
              ))}
            </ul>
          </div>

          {/* Right - Visual */}
          <div className="relative" data-testid="value-visual">
            <div className="rounded-lg overflow-hidden shadow-lg">
              <img 
                src={beforeAfterImage} 
                alt="Professional interior design" 
                className="w-full h-[400px] object-cover"
                data-testid="img-value-prop"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
