...
Ten reusable roles are defined in `.claude/agents/` and discovered
...
|---|---|---|---|
| `tech-lead` | Opus | high | Architecture, service boundaries, contract arbitration, ADRs |
| `ai-ml-lead` | Opus | high | Model selection, evaluation methodology, **all gate sign-offs** |
| `code-reviewer` | Opus | high | Review of code, architecture, and documents. Read-only |
| `delivery-coordinator` | Sonnet | medium | Work-packet decomposition, routing, phase and gate status |
...
| `ml-notebook-engineer` | Sonnet | medium | R/V/G/D notebooks, evaluation runs, logic extraction |
| `contracts-qa-steward` | Sonnet | medium | `contracts/v1`, suite runner, cross-service consistency |
| `research-scout` | Haiku | low | Fast read-only doc retrieval with citations |
| `packet-validator` | Haiku | low | Six-point completeness check on a packet before it is assigned |

The four leadership roles carry `memory: project`, so architectural
decisions, gate status, and recurring review findings accumulate across
sessions. Engineers deliberately do not — their scope is one work packet.

Validate any change with `claude plugin validate .claude/agents`.
