    ...
    QUIZ_ROOM_TYPES.forEach(roomType => {
        QUIZ_STYLES.forEach(style => {
            ...
            const matchingProducts = allProducts.filter((p) => // Explicitly type p
             p.roomType?.some((rt) => rt.toLowerCase() === roomType.toLowerCase()) &&
                p.designStyle?.some((ds) => ds.toLowerCase().includes(style.toLowerCase())));
...
//# sourceMappingURL=routes-mapping-analysis.js.map