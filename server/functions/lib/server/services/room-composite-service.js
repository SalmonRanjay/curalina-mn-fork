 ...
 */
export async function compositeWithSmoothEdges(originalRoomBase64, renderedImageBase64, threshold = 45, // RAISED from 25 to ignore wall changes
edgeBlur = 1 // REDUCED from 2 to minimize blur
) {
    try {
        ...
        for (let i = 0; i < pixelCount; i++) {
            ...
            const diff = Math.max(Math.abs(oR - rR), Math.abs(oG - rG), Math.abs(oB - rB));
            if (diff > threshold) {
        ...
        for (let i = 0; i < pixelCount; i++) {
            if (processedMask[i] > 0 && regionLabels[i] === 0) {
                ...
                while (stack.length > 0) {
                    const idx = stack.pop();
                    if (regionLabels[idx] !== 0)
                        continue;
                    if (processedMask[idx] === 0)
                        continue;
 ...
 */
export async function compositeHardBinary(originalRoomBase64, renderedImageBase64, threshold = 50) {
    try {
...
//# sourceMappingURL=room-composite-service.js.map