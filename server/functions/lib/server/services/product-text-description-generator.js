 ...
 */
export function generateRichProductDescription(product) {
    const sections = [];
    ...
    if (product.materials && product.materials.length > 0) {
        ...
        if (product.materials.some((m) => m.toLowerCase().includes('wood'))) {
            materialElements.push('with natural wood grain patterns adding organic warmth');
        }
        if (product.materials.some((m) => m.toLowerCase().includes('metal'))) {
            materialElements.push('metal components providing structural support with smooth finishes');
        }
        if (product.materials.some((m) => m.toLowerCase().includes('fabric') || m.toLowerCase().includes('upholstery') || m.toLowerCase().includes('linen'))) {
            materialElements.push('textile surfaces offering soft texture and comfort');
        }
        if (product.materials.some((m) => m.toLowerCase().includes('leather'))) {
            materialElements.push('leather upholstery with smooth grain and natural character');
        }
        if (product.materials.some((m) => m.toLowerCase().includes('glass'))) {
    ...
    if (product.colors && product.colors.length > 0) {
        ...
        const hasDark = product.colors.some((c) => darkColors.some((dc) => c.toLowerCase().includes(dc)));
        const hasLight = product.colors.some((c) => lightColors.some((lc) => c.toLowerCase().includes(lc)));
        if (hasDark && hasLight) {
    ...
    if (product.roomType && product.roomType.length > 0) {
        const rooms = product.roomType.map((room) => {
            const roomLower = room.toLowerCase();
            if (roomLower.includes('living'))
                return 'living rooms';
            if (roomLower.includes('bedroom'))
                return 'bedrooms';
            if (roomLower.includes('dining'))
                return 'dining spaces';
            if (roomLower.includes('office') || roomLower.includes('study'))
                return 'home offices';
            if (roomLower.includes('entryway') || roomLower.includes('foyer'))
                return 'entryways';
            return room;
        });
 ...
 */
export function batchGenerateDescriptions(products) {
...
//# sourceMappingURL=product-text-description-generator.js.map