...
export async function buildRenderProductSnapshots(params) {
    ...
    const deduplicatedProducts = Array.from(new Map(productsWithPlacement.map(p => [p.sku, p])).values());
    ...
    for (const productWithPlacement of deduplicatedProducts) {
        ...
        const snapshot = {
            ...
            dimensions: fullProduct.dimensions || null,
...
export function buildRenderEvent(renderId, eventType, details, actorUserId) {
    const eventMessages = {
...
export async function ingestRenderSnapshot(params, lifecycleEvents) {
    const { render } = params;
    try {
        const productSnapshots = await buildRenderProductSnapshots(params);
        await curalinaStorage.ingestRenderSnapshot(render.id, productSnapshots, lifecycleEvents);
        ...
        const failedEvent = buildRenderEvent(render.id, 'ingestion_failed', error instanceof Error ? error.message : 'Unknown ingestion error');
        try {
...
//# sourceMappingURL=render-ingestion.js.map