 ...
 */
function selectReferenceImages(products, maxReferences = 3) {
    const references = [];
 ...
 */
export async function refineRenderWithControlNetReference(params) {
 ...
 */
export function validateFurnitureFit(product, roomDimensions, placementPosition // As percentage of room width/depth
) {
 ...
 */
export function generateQCReport(products, roomDimensions) {
    const issues = [];
    const warnings = [];
    const recommendations = [];
    let totalScore = 0;
    for (const product of products) {
        const fitScore = validateFurnitureFit(product, roomDimensions, { x: 0.5, y: 0.5 } // Center position for overall assessment
        );
        totalScore += fitScore;
        if (fitScore < 50) {
 ...
 */
export function createQCOptimizedPrompt(basePrompt, productSkus, products) {
 ...
 */
export async function applyQCRefinement(baseImageData, prompt, productImages, layoutMask) {
...
//# sourceMappingURL=stability-ai-qc.js.map