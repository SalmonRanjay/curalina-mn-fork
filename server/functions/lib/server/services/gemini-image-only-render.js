 ...
 */
async function analyzeRoomWithGeminiVision(roomImageBase64) {
    const emptyResult = {
 ...
 */
async function executeFinalAlignmentPass(originalRoomBase64, furnishedRenderBase64, roomArchitectureAnalysis, productNames) {
    try {
        console.log(`\n🎯 STEP 3: FINAL ALIGNMENT PASS`);
 ...
 */
async function executeFinalAlignmentPassOpenAI(originalRoomBase64, furnishedRenderBase64, roomArchitectureAnalysis, productNames) {
    try {
        ...
        const originalFile = await toFile(Readable.from(originalBuffer), 'original_room.png', { type: 'image/png' });
        const renderFile = await toFile(Readable.from(renderBuffer), 'furniture_render.png', { type: 'image/png' });
        const productList = productNames.join(', ');
 ...
 */
function detectEmptyRoom(analysisText) {
 ...
 */
export async function validateRender(renderBase64, expectedProducts, originalRoomBase64) {
...
- Be strict about visibility - this is critical for e-commerce accuracy`
        ...
        const overallScore = Math.round((parsed.architecturalIntegrityScore || 0) * 0.25 +
        ...
        const hasSevereOcclusion = occludedProducts.some((p) => p.visibilityPercent < 60);
        ...
        if (!isValid) {
            const reasons = [];
            if (overallScore < 75)
                reasons.push(`low overall score (${overallScore}/75 required)`);
            if (productPlacementRate < 1.0)
                reasons.push(`missing products (${Math.round(productPlacementRate * 100)}% visible, 100% required)`);
            if (hasMissingProducts)
                reasons.push(`${parsed.missingProducts?.length} products missing: ${parsed.missingProducts?.join(', ')}`);
            if (occlusionScore < 80)
                reasons.push(`occlusion issues (score ${occlusionScore}/80 required)`);
            if (hasSevereOcclusion)
                reasons.push('severe occlusion (<60% visibility on some products)');
 ...
 */
function parseRoomDimensions(analysisText) {
    const result = {
        lengthFeet: null,
        widthFeet: null,
        ceilingHeightFeet: null,
    ...
    try {
        ...
        if (lengthMatch)
            result.lengthFeet = parseFloat(lengthMatch[1]);
        const widthMatch = analysisText.match(/Room width:\s*(\d+(?:\.\d+)?)\s*(?:feet|ft|')/i);
        if (widthMatch)
            result.widthFeet = parseFloat(widthMatch[1]);
        ...
        if (areaMatch)
            result.usableFloorArea = areaMatch[1].toLowerCase();
 ...
 */
function getPlacementContext(product) {
 ...
 */
function selectBestProductImage(product, context) {
    ...
    const urlContains = (url, patterns) => {
 ...
 */
function buildConciseVisualLabel(product) {
    const parts = [];
 ...
 */
function selectProductFrontViewImages(products) {
    const selected = [];
    const dropped = [];
    for (const product of products) {
        ...
        if (imageUrl && isValidImageUrl(imageUrl)) {
            ...
            if (lowerUrl.includes('front'))
                viewType = 'front';
            else if (lowerUrl.includes('side') || lowerUrl.includes('profile'))
                viewType = 'side';
            else if (lowerUrl.includes('angle') || lowerUrl.includes('3-4') || lowerUrl.includes('three'))
                viewType = 'angle';
 ...
 */
function detectFunctionalCategory(product) {
    const name = product.name.toLowerCase();
    const desc = (product.description || '').toLowerCase();
    const combined = `${name} ${desc}`;
    if (combined.includes('sofa') || combined.includes('couch') || combined.includes('sectional'))
        return 'Seating';
    if (combined.includes('chair') || combined.includes('armchair') || combined.includes('accent chair'))
        return 'Seating';
    if (combined.includes('coffee table'))
        return 'Coffee Table';
    if (combined.includes('side table') || combined.includes('end table') || combined.includes('accent table'))
        return 'Side Table';
    if (combined.includes('dining table'))
        return 'Dining Table';
    if (combined.includes('console') || combined.includes('sideboard') || combined.includes('credenza'))
        return 'Console';
    if (combined.includes('bookshelf') || combined.includes('bookcase') || combined.includes('shelf'))
        return 'Storage';
    if (combined.includes('cabinet') || combined.includes('dresser') || combined.includes('chest'))
        return 'Storage';
    if (combined.includes('floor lamp'))
        return 'Floor Lamp';
    if (combined.includes('table lamp'))
        return 'Table Lamp';
    if (combined.includes('pendant') || combined.includes('chandelier'))
        return 'Pendant Light';
    if (combined.includes('lamp'))
        return 'Lighting';
    if (combined.includes('rug') || combined.includes('carpet'))
        return 'Rug';
    if (combined.includes('mirror'))
        return 'Mirror';
    if (combined.includes('plant') || combined.includes('vase') || combined.includes('art'))
        return 'Decor';
    if (combined.includes('ottoman') || combined.includes('pouf'))
        return 'Ottoman';
    if (combined.includes('bench'))
        return 'Bench';
    if (combined.includes('bed') || combined.includes('headboard'))
        return 'Bed';
    if (combined.includes('nightstand') || combined.includes('bedside'))
        return 'Nightstand';
    return 'Furniture';
}
/**
 ...
 */
function getPlacementDirective(category, productIndex, totalProducts) {
 ...
 */
function getProductVisualDetails(product) {
    const details = [];
    // Add colors
    if (product.colors && product.colors.length > 0) {
 ...
 */
function buildAnchorCompositePrompt(products, productRefs, roomType, stylePreference, hasRoomImage, floorPlanAnalysis, placementInstructions, roomDescription, parsedRoomData) {
 ...
 */
export async function generateImageOnlyRender(params) {
    ...
    try {
        ...
        const prompt = buildAnchorCompositePrompt(limitedProducts, productImageRefs.filter(ref => productSkusSentToAI.includes(ref.sku)), roomType, stylePreference, !!roomImage, floorPlanAnalysis, placementInstructions, params.roomDescription, params.parsedRoomData);
 ...
 */
async function processGeminiResponse(response, productsUsed, productsSentToAI) {
    try {
 ...
 */
async function generateBatchedTextToImageRender(params, products) {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        const batchPrompt = buildBatchedTextToImagePrompt(batch, productImageRefs.filter(ref => batchSkus.includes(ref.sku)), roomType || 'living room', stylePreference || 'Modern', isFirstBatch, batchIndex, batches.length, seatingCount // Pass dining chair count for multiplied rendering
        );
        // Build parts array
        const parts = [];
 ...
 */
function buildBatchedTextToImagePrompt(products, productRefs, roomType, stylePreference, isFirstBatch, batchIndex, totalBatches, seatingCount // Number of dining chairs to render
) {
    const style = stylePreference || 'Modern';
    const room = roomType || 'living room';
    ...
    const diningChairRefs = productRefs.filter(ref => {
        ...
        if (name.includes('dining') && name.includes('chair'))
            return true;
        // Chair in a dining room context (not accent/arm chair)
        if (isDiningRoom && name.includes('chair') &&
            !name.includes('accent') && !name.includes('arm') && !name.includes('lounge'))
            return true;
        return false;
    });
    ...
    const getChairDistribution = (total) => {
        if (diningChairRefs.length === 0)
            return '';
 ...
 */
function getCategoryWeightForBatching(product) {
    const name = product.name.toLowerCase();
    ...
    if (name.includes('cabinet') || name.includes('sideboard') || name.includes('buffet'))
        return 80;
    if (name.includes('bookcase') || name.includes('shelf'))
        return 75;
    if (name.includes('console table') || name.includes('media console'))
        return 70;
    // Tier 3: Tables and rugs
    if (name.includes('coffee table'))
        return 65;
    if (name.includes('dining chair'))
        return 60;
    if (name.includes('rug'))
        return 60;
    // Tier 4: Accent seating
    if (name.includes('accent chair') || name.includes('armchair') || name.includes('chair'))
        return 55;
    if (name.includes('ottoman') || name.includes('bench'))
        return 50;
    // Tier 5: Small tables
    if (name.includes('side table') || name.includes('end table') || name.includes('nightstand'))
        return 40;
    // Tier 6: Lighting
    if (name.includes('lamp') || name.includes('pendant') || name.includes('chandelier'))
        return 30;
    // Tier 7: Small decor (last batch)
    if (name.includes('vase') || name.includes('bowl') || name.includes('pedestal'))
        return 20;
    if (text.includes('decor') || text.includes('accessory') || text.includes('accent'))
        return 15;
 ...
 */
function orderProductsForBatching(products) {
 ...
 */
function getCategoryWeight(product) {
    const name = product.name.toLowerCase();
    ...
    if (name.includes('sofa') || name.includes('sectional') || name.includes('couch'))
        return 100;
    // Tier 1.5: Beds (95)
    if (name.includes('bed') && !name.includes('daybed'))
        return 95;
    // Tier 2: Dining tables (90)
    if (name.includes('dining table') || (name.includes('table') && name.includes('dining')))
        return 90;
    // Tier 3: Large storage (80)
    if (name.includes('cabinet') || name.includes('sideboard') || name.includes('buffet') ||
        name.includes('armoire') || name.includes('wardrobe') || name.includes('hutch'))
        return 80;
    // Tier 3.5: Media furniture (75)
    if (name.includes('media') || name.includes('entertainment') || name.includes('tv stand') ||
        name.includes('bookcase') || name.includes('bookshelf') || name.includes('shelving'))
        return 75;
    // Tier 4: Console tables (70)
    if (name.includes('console'))
        return 70;
    // Tier 5: Coffee tables (65)
    if (name.includes('coffee table') || (name.includes('coffee') && name.includes('table')))
        return 65;
    // Tier 5.5: Dining chairs and rugs (60)
    if (name.includes('dining chair') || name.includes('rug') || name.includes('carpet'))
        return 60;
    // Tier 6: Accent seating (55)
    if (name.includes('accent chair') || name.includes('armchair') || name.includes('lounge chair'))
        return 55;
    // Tier 6.5: General chairs and ottomans (50)
    if (name.includes('chair') || name.includes('ottoman') || name.includes('daybed'))
        return 50;
    // Tier 7: Benches and stools (45)
    if (name.includes('bench') || name.includes('stool'))
        return 45;
    // Tier 8: Small tables (40)
    if (name.includes('side table') || name.includes('accent table') || name.includes('end table') ||
        name.includes('nightstand') || name.includes('night stand'))
        return 40;
    // Tier 8.5: Pedestal sets (35)
    if (name.includes('pedestal'))
        return 35;
    // Tier 9: Floor lamps (30)
    if (name.includes('floor lamp'))
        return 30;
    // Tier 10: Table lamps and pendants (25)
    if (name.includes('lamp') || name.includes('pendant') || name.includes('chandelier') ||
        name.includes('sconce') || name.includes('light'))
        return 25;
    // Tier 11: Decor (15)
    if (name.includes('decor') || name.includes('vase') || name.includes('planter') ||
        name.includes('sculpture') || name.includes('figurine'))
        return 15;
    // Tier 12: Art and mirrors (10)
    if (name.includes('art') || name.includes('mirror') || name.includes('frame') ||
        name.includes('picture') || name.includes('print'))
        return 10;
 ...
 */
export async function generateMultiStepRender(params) {
    ...
    try {
        ...
        for (let batchIndex = 0; batchIndex < productBatches.length; batchIndex++) {
            ...
            const batchResult = await executeProductBatchPass(currentAnchor, batch, style, room, batchIndex, productBatches.length, originalRoomBase64, // Original room image for architecture preservation
        ...
        if (allProductsSentToAI.length > 0 && currentAnchor && roomArchitectureAnalysis) {
            ...
            const alignmentResult = useOpenAIAlignment
                ? await executeFinalAlignmentPassOpenAI(trueOriginalBase64, // Original user's room
                ...
                : await executeFinalAlignmentPass(trueOriginalBase64, // Original user's room
                currentAnchor, // Generated render with furniture
                roomArchitectureAnalysis, // Detailed room analysis
                productNames // Products to composite
                );
 ...
 */
export async function generateOneByOneRender(params) {
    ...
    try {
        ...
        for (let i = 0; i < instances.length; i++) {
            ...
            const singleResult = await executeSingleProductPass(currentAnchor, instance, style, room, roomArchitectureAnalysis);
        ...
        if (allProductsSentToAI.length > 0) {
            ...
            const alignmentResult = await executeFinalAlignmentPassOpenAI(trueOriginalBase64, currentAnchor, roomArchitectureAnalysis, productNames);
            if (alignmentResult.success && alignmentResult.imageBase64) {
                finalImage = alignmentResult.imageBase64;
 ...
 */
async function executeSingleProductPass(anchorImage, instance, style, room, roomArchitecture) {
    try {
 ...
 */
function selectSingleProductImage(product) {
    if (!product.images || !Array.isArray(product.images) || product.images.length === 0) {
        return null;
    }
    const images = product.images.filter(img => img && typeof img === 'string' && img.trim().length > 0);
 ...
 */
function buildSingleProductPrompt(instance, style, room, roomArchitecture) {
    const product = instance.product;
 ...
 */
async function executeRoomLockPass(roomImageUrl, style, room) {
    try {
        // Fetch room image
 ...
 */
async function executeProductBatchPass(anchorBase64, products, style, room, batchIndex, totalBatches, originalRoomBase64, // Original room image for architecture reference
roomArchitectureAnalysis, // Detailed room analysis from Gemini Vision
roomDimensions, // Estimated room dimensions
seatingCount // Number of dining chairs to render (for dining rooms)
) {
    try {
        // Get product images
...
//# sourceMappingURL=gemini-image-only-render.js.map