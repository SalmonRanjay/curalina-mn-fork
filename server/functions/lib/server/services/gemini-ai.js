...
import { selectProductsWithComposition, detectFunctionalCategory, getRoomTemplate } from './room-composition-service';
 ...
 */
function normalizeAndTokenizeStyle(style) {
 ...
 */
function stylesMatch(quizStyle, productStyle) {
    ...
    return quizTokens.some(qt => productTokens.some(pt => qt.includes(pt) || pt.includes(qt)));
}
/**
 ...
 */
function calculateVisualSimilarityScore(product, quiz) {
 ...
 */
function hasAIVisualDescription(product) {
 ...
 */
function canUseForAIRendering(product) {
 ...
 */
export function filterProductsByQuiz(products, quiz) {
    ...
    const strictlyFiltered = products.filter(product => {
        ...
        if (!canUseForAIRendering(product))
            return false;
        // Filter by room type (relaxed for essential furniture)
        if (product.roomType && product.roomType.length > 0) {
            const roomMatch = product.roomType.some(rt => rt.toLowerCase().includes(quiz.roomType.toLowerCase()) ||
        ...
        if (hasDesignStyle || hasStyleTags) {
            ...
            if (quiz.styles && quiz.styles.length > 0) {
                ...
                if (hasDesignStyle) {
                    styleMatch = quiz.styles.some(quizStyle => product.designStyle.some(ds => stylesMatch(quizStyle, ds)));
                }
                // Fallback to styleTags if designStyle didn't match
                if (!styleMatch && hasStyleTags) {
                    styleMatch = quiz.styles.some(quizStyle => product.styleTags.some(st => stylesMatch(quizStyle, st)));
                }
            }
            if (!styleMatch)
                return false;
 ...
 */
function getRoomEssentialCategories(roomType) {
 ...
 */
export async function selectProductsWithAI(products, quiz) {
    try {
        ...
        const productList = products.map(p => {
            ...
            if (quiz.keyFeatures && quiz.keyFeatures.length > 0 &&
                ...
                const matchingFeatures = quiz.keyFeatures.filter(qf => p.keyFeatures.some(pf => pf.toLowerCase().includes(qf.toLowerCase()) ||
...
[
  {
    ...
    "reasoning": "detailed explanation of feature match and design fit"
        ...
        const compositionResult = await selectProductsWithComposition(quiz.roomType, products, // Pass all available products for better selection
        quiz, 8 // Max 7-8 products (matches successful "Nano Banana" approach in Google AI Studio)
        );
        ...
        const budgetValidatedProducts = validateAndAdjustForBudget(compositionProducts, products, budgetMax, quiz);
 ...
 */
function validateAndAdjustForBudget(selectedProducts, allProducts, budgetMax, quiz) {
 ...
 */
function generateScaleConstraints(products) {
    // Helper to extract dimension value supporting both key formats
    const getDim = (dims, key) => {
    ...
    productsWithDims.forEach(product => {
        ...
        if (width)
            parts.push(`${width}${unit} wide`);
        if (depth)
            parts.push(`${depth}${unit} deep`);
        if (height)
            parts.push(`${height}${unit} tall`);
        if (parts.length > 0) {
            scaleText += `• ${product.name}: ${parts.join(', ')}\n`;
        }
    });
    // Generate comparative size relationships for key furniture pairs
    const sofas = productsWithDims.filter(p => p.functionalCategory === 'seating' || p.name.toLowerCase().includes('sofa') || p.name.toLowerCase().includes('chair'));
    const tables = productsWithDims.filter(p => p.functionalCategory === 'surfaces' || p.name.toLowerCase().includes('table'));
...
function buildProductSpecifications(product) {
    ...
    if (product.dimensions) {
        ...
        const dimParts = [];
        if (dims.w)
            dimParts.push(`W: ${dims.w}${dims.unit || '"'}`);
        if (dims.d)
            dimParts.push(`D: ${dims.d}${dims.unit || '"'}`);
        if (dims.h)
            dimParts.push(`H: ${dims.h}${dims.unit || '"'}`);
        if (dimParts.length > 0) {
            specs.push(`DIMENSIONS: ${dimParts.join(' × ')}`);
        }
        // Additional dimension details
        if (dims.armWidth)
            specs.push(`Arm Width: ${dims.armWidth}${dims.unit || '"'}`);
        if (dims.armDepth)
            specs.push(`Arm Depth: ${dims.armDepth}${dims.unit || '"'}`);
        if (dims.seatWidth)
            specs.push(`Seat Width: ${dims.seatWidth}${dims.unit || '"'}`);
        if (dims.seatDepth)
            specs.push(`Seat Depth: ${dims.seatDepth}${dims.unit || '"'}`);
        if (dims.seatHeight)
            specs.push(`Seat Height: ${dims.seatHeight}${dims.unit || '"'}`);
        if (dims.volume)
            specs.push(`Volume: ${dims.volume}`);
        if (dims.doorWidth)
            specs.push(`Door Width: ${dims.doorWidth}${dims.unit || '"'}`);
        if (dims.doorThickness)
            specs.push(`Door Thickness: ${dims.doorThickness}${dims.unit || '"'}`);
        if (dims.doorHeight)
            specs.push(`Door Height: ${dims.doorHeight}${dims.unit || '"'}`);
        if (dims.legBaseDepth1)
            specs.push(`Leg/Base Depth: ${dims.legBaseDepth1}${dims.unit || '"'}`);
        if (dims.legBaseHeight1)
            specs.push(`Leg/Base Height: ${dims.legBaseHeight1}${dims.unit || '"'}`);
        if (dims.legBaseWidth1)
            specs.push(`Leg/Base Width: ${dims.legBaseWidth1}${dims.unit || '"'}`);
        if (dims.tabletopThickness)
            specs.push(`Tabletop Thickness: ${dims.tabletopThickness}${dims.unit || '"'}`);
        if (dims.shapeType)
            specs.push(`Shape: ${dims.shapeType}`);
    }
    // Seating capacity
    if (product.seating) {
    ...
    if (product.shipping) {
        ...
        if (ship.deliveryOptions) {
            ...
            if (deliveryOpts)
                specs.push(`Delivery Options: ${deliveryOpts}`);
        }
    ...
    if (product.tags) {
        const cleanTags = Array.isArray(product.tags)
            ? Array.from(new Set(product.tags.filter(Boolean).map((t) => String(t).trim())))
 ...
 */
function condenseVisualDescription(fullDescription, productName) {
 ...
 */
function getBestVisualDescription(product) {
    ...
    if (product.completeProductDescription && product.completeProductDescription.trim().length > 0) {
        ...
        const frontViewIdx = lines.findIndex((l) => l.includes('FRONT VIEW:'));
        ...
        const firstParagraph = lines.find((l) => l.length > 50);
        if (firstParagraph) {
 ...
 */
export function attachPlacementMetadata(selectedProducts, ledgerEntry, roomType, allProducts) {
    ...
    if (ledgerEntry?.selectionRationale) {
        ...
        if (rationale.essentials) {
            for (const [category, data] of Object.entries(rationale.essentials)) {
        ...
        if (rationale.complementary) {
            for (const [category, data] of Object.entries(rationale.complementary)) {
                if (data.selectedProducts && Array.isArray(data.selectedProducts)) {
                    for (const sku of data.selectedProducts) {
                        ledgerMetadata[sku] = { category, priority: currentPriority };
                        currentPriority++;
    ...
    return selectedProducts.map((product, index) => {
        ...
        const fullProduct = allProducts.find((p) => p.sku === product.sku);
        if (fullProduct) {
            ...
            if (template) {
                ...
                if (template.essentials[category]) {
                    priority = template.essentials[category].priority;
                }
                else if (template.complementary && template.complementary[category]) {
                    priority = template.complementary[category].priority;
                }
            }
 ...
 */
function getSemanticPosition(zoneId, position, anchorPoint) {
 ...
 */
function getDirectionalGuidance(placement, allPlacements) {
 ...
 */
function generateZoneBasedPlacementMatrix(selectedProducts, placements, matrixHeader) {
...
function getAnchorDescription(anchorPoint) {
    switch (anchorPoint) {
        case 'center':
 ...
 */
export function generatePlacementMatrix(selectedProducts, roomType, roomAnalysis, floorPlanAnalysis, placements) {
    const matrix = [];
    matrix.push(`\n\n═══════════════════════════════════════════════════════════════════════════`);
 ...
 */
function getAnchorZone(product, roomType, hasWindows, hasDoors) {
 ...
 */
function getPositioningRules(product, roomType) {
    const category = product.functionalCategory?.toLowerCase() || '';
    const name = product.name.toLowerCase();
    if (category.includes('seating') && (name.includes('sofa') || name.includes('sectional'))) {
 ...
 */
function getClearanceRules(product) {
    const category = product.functionalCategory?.toLowerCase() || '';
    const name = product.name.toLowerCase();
 ...
 */
function getAdjacencyRules(product, allProducts, roomType) {
    const category = product.functionalCategory?.toLowerCase() || '';
    const name = product.name.toLowerCase();
 ...
 */
function getRelativePosition(product, essentials, roomType) {
    const category = product.functionalCategory?.toLowerCase() || '';
    const name = product.name.toLowerCase();
    // Find key essentials
    const primarySeating = essentials.find(p => p.name.toLowerCase().includes('sofa') || p.name.toLowerCase().includes('sectional'));
 ...
 */
export function buildPromptFromQuiz(quiz, selectedProducts, roomAnalysis, floorPlanAnalysis, placements) {
 ...
 */
export async function analyzeVibeImages(imageUrls) {
    try {
        ...
        const imageData = await Promise.all(imageUrls.map(async (url) => {
 ...
 */
export async function identifyVisibleProducts(imageDataUrl, selectedProducts) {
    try {
        ...
        const productList = selectedProducts.map((p, idx) => `${idx + 1}. ${p.name} (SKU: ${p.sku})`).join('\n');
...
Return your analysis as a JSON object with this format:
{
  "visibleSkus": ["SKU1", "SKU2", ...]
        ...
        const uniqueSkus = Array.from(new Set(visibleSkus));
 ...
 */
export async function generateInteriorImage(prompt, floorplanUrl, roomAnalysis, floorPlanAnalysis, productImages, layoutMask) {
    try {
        ...
        if (!floorplanUrl) {
            ...
            const parts = [{ text: prompt }, ...productImageParts];
            const response = await ai.models.generateContent({
            ...
            if (qcEnabled && hasApiKey) {
                ...
                if (!qcEnabled)
                    console.log("ℹ️ QC disabled by admin settings");
                if (!hasApiKey)
                    console.log("ℹ️ Stability AI API key not configured");
            }
            return baseImage;
        }
        ...
        if (!photoMimeType.startsWith('image/')) {
            ...
            if (urlLower.endsWith('.png'))
                ...
                photoMimeType = 'image/jpeg'; // default fallback
        }
 ...
 * @returns Base64 data URL (data:image/png;base64,...)
 */
export async function generateInteriorImageWithOpenAI(prompt) {
    try {
 ...
 */
function truncatePromptForStability(prompt, maxLength = 9500) {
 ...
 */
async function generateWithStabilityAI(imageBuffer, prompt, mimeType) {
 ...
 */
export function extractProductSkus(quiz, availableProducts) {
    // Filter products by style match - check if any quiz style matches any product tag
    const styleMatch = availableProducts.filter(p => quiz.styles && quiz.styles.length > 0 &&
        quiz.styles.some(quizStyle => p.styleTags.some(tag => tag.toLowerCase() === quizStyle.toLowerCase())));
    // Return up to 6 product SKUs
    return styleMatch
        .slice(0, 6)
        .map(p => p.sku);
}
export async function matchFoldersToProducts(folderNames, products) {
 ...
 */
export async function detectVisibleProducts(renderImageBase64, selectedProducts) {
    try {
...
If NO products are clearly visible, return an empty array: []`;
        ...
        const visibleSkus = JSON.parse(responseText);
...
//# sourceMappingURL=gemini-ai.js.map