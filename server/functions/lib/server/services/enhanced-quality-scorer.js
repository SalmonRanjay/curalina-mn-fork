export class EnhancedQualityScorer {
     ...
     */
    static scoreColorAccuracy(analysis) {
        ...
        if (hasHex)
            score += 50;
        if (colorFinishLength > 30)
            score += 30;
        if (colorFinishLength > 50)
            score += 20;
     ...
     */
    static scoreDimensionPrecision(analysis) {
        const dims = analysis.dimensions;
        if (!dims)
            return { score: 0, details: 'No dimensions provided' };
        let score = 0;
        ...
        if (dims.unit === 'inches' || dims.unit === 'cm') {
            score += 25;
        }
     ...
     */
    static scoreMaterialSpecificity(analysis) {
        const material = analysis.primaryMaterial || '';
        let score = 0;
        // Check for specific material types (not generic)
        if (/solid|wood|oak|walnut|maple|pine|ash/i.test(material))
            score += 20;
        if (/upholster|fabric|leather|linen|cotton|wool/i.test(material))
            score += 20;
        if (/metal|steel|brass|aluminum|iron|chrome/i.test(material))
            score += 20;
        if (/finish|lacquer|stain|varnish|gloss|matte|satin/i.test(material))
            score += 20;
        if (/poly|foam|cushion|padding/i.test(material))
            score += 20;
     ...
     */
    static scoreTextureDetail(analysis) {
        const geometry = analysis.keyGeometry || '';
        const viewDetails = analysis.viewSpecificDetails || '';
        let score = 0;
        if (/grain|weave|pattern|texture|finish|surface/i.test(geometry))
            score += 40;
        if (/grain|weave|pattern|texture|finish|surface/i.test(viewDetails))
            score += 30;
     ...
     */
    static scoreFeatureCompleteness(analysis) {
     ...
     */
    static scoreGeometryPrecision(analysis) {
        const geometry = analysis.keyGeometry || '';
        const formFactor = analysis.formFactor || '';
        let score = 0;
        // Check for shape descriptors
        if (/curved|linear|geometric|symmetrical|asymmetrical/i.test(geometry))
            score += 25;
        if (/tapered|splayed|angled|perpendicular/i.test(geometry))
            score += 25;
        if (geometry.length > 40)
            score += 25;
        if (formFactor.length > 15)
            score += 25;
     ...
     */
    static calculateMetrics(analysis) {
...
//# sourceMappingURL=enhanced-quality-scorer.js.map