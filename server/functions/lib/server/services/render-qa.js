...
import { normalizeDimensions } from './dimension-utils';
/**
 ...
 */
export async function validateRenderQuality(renderImageBase64, mimeType, expectedProducts, renderPrompt) {
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        ...
                        dimensionAccuracy: { type: "integer" },
                        productChecks: {
                            type: "array",
                            items: {
                                type: "object",
                                properties: {
                                    ...
                                    appearanceMatch: { type: "integer" },
                                    scaleAccuracy: { type: "integer" },
                                    dimensionAccuracy: { type: "integer" },
                                    placementCorrect: { type: "boolean" },
                                    notes: { type: "string" }
                                },
 ...
 */
function buildValidationPrompt(expectedProducts, renderPrompt) {
...
`;
    expectedProducts.forEach((product, index) => {
        ...
        if (product.dimensions) {
            ...
            const getDim = (key) => dims[key] || dims[key[0]];
            const parts = [];
 ...
 */
function parseJSONQAResponse(analysisText, expectedProducts) {
    try {
        ...
        const findProductByName = (productName, productColors) => {
            const nameLower = productName.toLowerCase();
            // Find all products matching the name
            const nameMatches = expectedProducts.filter(p => p.name.toLowerCase() === nameLower);
            ...
            if (productColors && productColors.length > 0) {
                ...
                const colorMatch = nameMatches.find(p => {
                    ...
                    return p.colors.some(pColor => productColors.some(checkColor => pColor.toLowerCase().includes(checkColor.toLowerCase()) ||
        ...
        if (parsed.productChecks && Array.isArray(parsed.productChecks)) {
            ...
            parsed.productChecks.forEach((check) => {
                ...
                let matchMethod = matchedProduct ? 'sku' : 'none';
        ...
        const unmatchedProducts = expectedProducts.filter(expected => !productChecks[expected.sku]);
 ...
 */
function parseQAResponse(analysisText, expectedProducts) {
    const issues = [];
    const productChecks = {};
    ...
    while ((match = productCheckRegex.exec(analysisText)) !== null) {
        ...
        const product = expectedProducts.find(p => p.name.toLowerCase() === productName.toLowerCase());
        if (product) {
        ...
        issueLines.forEach(line => {
            ...
            if (severityMatch && descriptionMatch) {
                issues.push({
                    ...
                    category: (categoryMatch?.[1]?.toLowerCase() || 'appearance'),
 ...
 */
export function shouldRegenerateRender(qaResults) {
    ...
    const hasColorMismatch = Object.values(qaResults.productChecks).some(check => check.found && check.colorMatch === false);
    if (hasColorMismatch) {
...
//# sourceMappingURL=render-qa.js.map