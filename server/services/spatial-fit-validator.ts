...
export interface ParsedRoomData {
  dimensions: {
    width: number;
    depth: number;
    height?: number; // Optional - may not always be provided
    unit: string;
    confidence: number;
  } | null;
  doorway: {
...
export type ValidationSeverity = 'BLOCK' | 'WARNING' | 'OK';

export interface SpatialValidationResult {
  severity: ValidationSeverity;
  canFit: boolean;
  canDeliverThroughDoor: boolean;
  hasAdequateClearance: boolean;
  messages: string[];
  details: {
    roomEnvelopeCheck?: string;
    doorwayCheck?: string;
 ...
 */
function canFitThroughDoorway(
  productDimensions: { w: number; d: number; h: number; unit: string },
  doorwayDimensions: { width: number; height: number; unit: string }
 ...
 */
function fitsInRoomEnvelope(
  productDimensions: { w: number; d: number; h: number; unit: string },
  roomDimensions: { width: number; depth: number; height?: number; unit: string }
): { fits: boolean; hasComfortableClearance: boolean; message: string } {
 ...
 */
export async function validateProductFit(
  product: Product,
  ...
  const details: SpatialValidationResult['details'] = {};
...
export async function validateProductsBatch(
  products: Product[],
  parsedRoomData: ParsedRoomData | null
): Promise<Map<string, SpatialValidationResult>> {
  const results = new Map<string, SpatialValidationResult>();
 ...
 */
export async function filterFittingProducts(
  products: Product[],
  parsedRoomData: ParsedRoomData | null
): Promise<{
  fitting: Product[];
  blocked: Array<{ product: Product; reason: string }>;
  warnings: Array<{ product: Product; messages: string[] }>;
}> {
  const fitting: Product[] = [];
  const blocked: Array<{ product: Product; reason: string }> = [];
  const warnings: Array<{ product: Product; messages: string[] }> = [];
