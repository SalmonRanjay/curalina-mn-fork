...
import type { Product, QuizResponse, CandidatePoolSnapshot, SelectionRationale } from '../../shared/schema';
 ...
 */
export function createCandidatePoolSnapshot(products: Product[]): CandidatePoolSnapshot {
  return products.map(p => ({
    ...
    hasValidImage: !!((p.images && p.images.length > 0) || p.frontViewImageUrl),
 ...
 */
export function initializeSelectionRationale(): SelectionRationale {
  return {
    essentials: {},
    complementary: {},
    excluded: [],
...
export function recordSelection(
  ...
  product: Product,
  reason: string,
  rules: { min: number; max: number; priority: number }
): void {
 ...
 */
export function calculateDiversityScore(rationale: SelectionRationale): number {
  const totalCategories = 
    Object.keys(rationale.essentials).length + 
    Object.keys(rationale.complementary).length;
  
  if (totalCategories === 0) return 0;
  
  const totalProducts = 
    Object.values(rationale.essentials).reduce((sum, cat) => sum + cat.selectedProducts.length, 0) +
 ...
 */
export function generateCompositionOrder(
  rationale: SelectionRationale
): string[] {
  const order: string[] = [];
