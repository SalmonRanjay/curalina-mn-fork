...
// TypeScript interface for normalized dimensions
export interface NormalizedDimensions {
  ...
  // Table-specific
  legBaseDepth1?: number;
  legBaseHeight1?: number;
  legBaseWidth1?: number;
  tabletopThickness?: number;
  
  // Additional attributes
  shapeType?: string;
  unit: string; // Default to 'inches'
}
 ...
 */
const CATEGORY_DIMENSION_MAP: Record<string, (keyof NormalizedDimensions)[]> = {
  // Seating furniture
 ...
 */
export function normalizeDimensions(product: Partial<Product> | { dimensions?: any; name?: string; description?: string }): NormalizedDimensions | null {
  ...
  const getDimValue = (shortKey: string, longKey: string): number | undefined => {
  ...
  const normalized: NormalizedDimensions = {
  ...
  if (depth !== undefined) normalized.depth = depth;
  if (height !== undefined) normalized.height = height;
  
  // Seating dimensions
  if (dims.seatWidth !== undefined && dims.seatWidth !== null) normalized.seatWidth = parseFloat(dims.seatWidth);
  if (dims.seatDepth !== undefined && dims.seatDepth !== null) normalized.seatDepth = parseFloat(dims.seatDepth);
  if (dims.seatHeight !== undefined && dims.seatHeight !== null) normalized.seatHeight = parseFloat(dims.seatHeight);
  if (dims.armWidth !== undefined && dims.armWidth !== null) normalized.armWidth = parseFloat(dims.armWidth);
  if (dims.armDepth !== undefined && dims.armDepth !== null) normalized.armDepth = parseFloat(dims.armDepth);
  
  // Cabinet/Storage dimensions
  if (dims.doorWidth !== undefined && dims.doorWidth !== null) normalized.doorWidth = parseFloat(dims.doorWidth);
  if (dims.doorHeight !== undefined && dims.doorHeight !== null) normalized.doorHeight = parseFloat(dims.doorHeight);
  if (dims.doorThickness !== undefined && dims.doorThickness !== null) normalized.doorThickness = parseFloat(dims.doorThickness);
  if (dims.volume !== undefined && dims.volume !== null) normalized.volume = parseFloat(dims.volume);
  
  // Table dimensions
  if (dims.legBaseDepth1 !== undefined && dims.legBaseDepth1 !== null) normalized.legBaseDepth1 = parseFloat(dims.legBaseDepth1);
  if (dims.legBaseHeight1 !== undefined && dims.legBaseHeight1 !== null) normalized.legBaseHeight1 = parseFloat(dims.legBaseHeight1);
  if (dims.legBaseWidth1 !== undefined && dims.legBaseWidth1 !== null) normalized.legBaseWidth1 = parseFloat(dims.legBaseWidth1);
  if (dims.tabletopThickness !== undefined && dims.tabletopThickness !== null) normalized.tabletopThickness = parseFloat(dims.tabletopThickness);
  
  // Additional attributes
  if (dims.shapeType) normalized.shapeType = dims.shapeType;
 ...
 */
export function detectFurnitureCategory(product: Partial<Product> | { name: string; description?: string | null }): string {
 ...
 */
export function filterDimensionsByCategory(
  dimensions: NormalizedDimensions,
  category: string
): Partial<NormalizedDimensions> {
  const relevantKeys = CATEGORY_DIMENSION_MAP[category] || CATEGORY_DIMENSION_MAP['default'];
  
  const filtered: Partial<NormalizedDimensions> = {
    unit: dimensions.unit
  };
  
  relevantKeys.forEach(key => {
    if (dimensions[key] !== undefined && dimensions[key] !== null) {
      (filtered as any)[key] = dimensions[key];
    }
  });
 ...
 */
const SCALE_REFERENCES: Record<string, (value: number) => string | null> = {
  // Overall furniture dimensions
  width: (w: number) => {
    if (w < 30) return 'compact accent piece';
    if (w < 48) return 'armchair size';
    if (w < 72) return 'loveseat width';
    if (w < 96) return 'standard sofa width';
    if (w < 120) return 'large sofa width';
    return 'sectional width';
  },
  
  height: (h: number) => {
    if (h < 18) return 'low-profile';
    if (h < 30) return 'coffee table height';
    if (h < 36) return 'standard seating height';
    if (h < 42) return 'counter height';
    if (h < 80) return 'standard furniture height';
    return 'tall cabinet height';
  },
  
  seatHeight: (sh: number) => {
    if (sh < 16) return 'low lounge seating';
    if (sh < 19) return 'standard seating height';
    if (sh < 25) return 'counter-height seating';
    return 'bar-height seating';
  },
  
  tabletopThickness: (tt: number) => {
    if (tt < 1) return 'sleek thin profile';
    if (tt < 2) return 'standard tabletop';
 ...
 */
export function buildDimensionPrompt(product: Partial<Product> | { dimensions?: any; name?: string; description?: string }): string | null {
  ...
  if (filtered.seatWidth !== undefined || filtered.seatDepth !== undefined) {
    ...
    if (seatDims) parts.push(`Seat: ${seatDims}"`);
  }
  ...
  if (filtered.armWidth !== undefined || filtered.armDepth !== undefined) {
    ...
    if (armDims) parts.push(`Arms: ${armDims}"`);
  }
  ...
  if (filtered.doorWidth !== undefined || filtered.doorHeight !== undefined) {
    ...
    if (doorDims) parts.push(`Doors: ${doorDims}"`);
  }
 ...
 * Works with both full Product objects and partial product selections
 */
export function buildDimensionSummary(product: Partial<Product> | { dimensions?: any; name?: string; description?: string }): string | null {
  const normalized = normalizeDimensions(product);
  ...
  if (category.includes('chair') || category.includes('sofa') || category.includes('sectional')) {
    if (filtered.seatHeight) parts.push(`seat ${filtered.seatHeight}"`);
  } else if (category.includes('table')) {
    if (filtered.tabletopThickness) parts.push(`${filtered.tabletopThickness}" top`);
  } else if (category.includes('cabinet') || category.includes('wardrobe')) {
    if (filtered.volume) parts.push(`${filtered.volume}cf storage`);
  }
 ...
 * Works with both full Product objects and partial product selections
 */
export function hasAdequateDimensionData(product: Partial<Product> | { dimensions?: any; name?: string }): boolean {
  const normalized = normalizeDimensions(product);
