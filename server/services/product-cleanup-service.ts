 ...
 */
async function hasValidImages(images: string[] | null): Promise<boolean> {
  ...
  const validationResults = await Promise.all(
    images.map(url => validateImageUrl(url))
  );
 ...
 */
export async function identifyProductsToCleanup(
  storage: ICuralinaStorage,
  options: {
    validateImages?: boolean; // If true, check if image URLs are accessible (slow)
  } = {}
): Promise<{
  ...
  const productsToDelete: Array<{ id: string; sku: string; reason: string }> = [];
...
export async function executeCleanup(
  ...
  deletedRelations: {
    cart_items: number;
    order_items: number;
    product_functional_categories: number;
    render_products: number;
    upload_jobs: number;
    visual_analysis_products: number;
  };
}> {
  let deleted = 0;
  let failed = 0;
 ...
 */
async function deleteProductWithDependencies(productId: string): Promise<{
  cart_items: number;
  order_items: number;
  product_functional_categories: number;
  render_products: number;
