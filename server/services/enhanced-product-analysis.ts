...
export interface TechnicalDeconstruction {
...
export async function analyzeProductForRendering(
  product: Product,
  imageUrl: string
): Promise<EnhancedProductAnalysis | null> {
  try {
    ...
    const enhancedAnalysis: EnhancedProductAnalysis = {
      technicalDeconstruction: {
...
function calculateAnalysisQuality(analysis: any): number {
  let score = 0;
  const td = analysis.technicalDeconstruction;

  if (td?.subjectCategory && td.subjectCategory.length > 10) score += 15;
  if (td?.geometryShape && td.geometryShape.length > 15) score += 15;
  if (td?.materialsTextures && td.materialsTextures.length > 20) score += 20;
  if (td?.hardwareDetails && td.hardwareDetails.length > 10) score += 15;
  if (td?.colorPalette && td.colorPalette.length >= 2) score += 10;
  if (td?.cameraAngle && td.cameraAngle.length > 5) score += 5;
  if (td?.lightingShadows && td.lightingShadows.length > 5) score += 5;

  if (analysis.generationPrompt && analysis.generationPrompt.length > 50) score += 10;
  if (analysis.renderingKeywords && analysis.renderingKeywords.length >= 6) score += 5;

  return Math.min(score, 100);
}

export function selectBestImageForEnhancedAnalysis(product: Product): { url: string; source: string } | null {
...
export interface BatchEnhancedAnalysisProgress {
  total: number;
  processed: number;
  successful: number;
  failed: number;
...
export async function runBatchEnhancedAnalysis(
  products: Product[],
  storage: any,
  options: { mode?: 'missing_only' | 'regenerate_all'; limit?: number } = {},
  progressCallback?: (progress: BatchEnhancedAnalysisProgress) => void
): Promise<BatchEnhancedAnalysisProgress> {
  ...
  if (mode === 'missing_only') {
    productsToProcess = products.filter(p => {
      const hasAnalysis = p.structuredAnalysis && 
        ...
        (p.structuredAnalysis as any).analysisVersion === '2.0-vertex-style';
      return !hasAnalysis;
    });
  }
  ...
  const progress: BatchEnhancedAnalysisProgress = {
...
export function buildRenderingPromptFromAnalysis(analysis: EnhancedProductAnalysis): string {
