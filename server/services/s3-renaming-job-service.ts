...
import type { S3RenamingJob, S3RenamingProduct, Product } from '@shared/schema';
import { renameS3Object, checkS3ObjectExists } from '../s3';

/**
 * Persistent S3 Renaming Job Service with priority queuing,
 * checkpoint saves, and automatic recovery
 */
 ...
 */
function fullyDecodeKey(key: string): string {
  let decoded = key;
  let prev;
...
class S3RenamingJobQueue {
   ...
   */
  private async saveCheckpoint(jobId: string, processed: number, successful: number, failed: number): Promise<void> {
 ...
 */
export async function createAndStartS3RenamingJob(
  productIds?: string[],
  options: {
    priority?: number;
    autoStart?: boolean;
  ...
  const productsNeedingRename = products.filter(p => 
 ...
 */
export async function resumeInterruptedS3Jobs(): Promise<void> {
 ...
 */
export async function getS3JobDetails(jobId: string) {
  const job = await curalinaStorage.getS3RenamingJob(jobId);
  if (!job) return null;
